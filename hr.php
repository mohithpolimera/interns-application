...
<section class="vh-100">
    ...
    <div class="row d-flex justify-content-center align-items-center h-100">
      <div class="col-md-8 col-lg-6 col-xl-9 offset-xl-1">
          <div class="divider d-flex align-items-center my-4">
            <h3 class="text-center fw-bold mx-3 mb-0">Statistics of Interns</h3>
          </div>
        ...
        <div class="content_box">
            <div id="tabs-1">
                <h3>Accepted Students List</h3>
                <br>
                <p>
                <?php
            $sql = "SELECT COUNT(*) AS total FROM kmrlform where status='accept'";
            ...
            $sql1 = "SELECT name,email,status FROM kmrlform where status='accept' LIMIT $start_from, $limit";
            ...
            while($row =$result1->fetch_assoc() ){
                ...
                $html .="<td><a href='profile.php?email=".$em."'>View Details</a></td></tr>";
                break;
            }
            while($row =$result1->fetch_assoc() ){
                $html .="<tr>";
        ...
        ?>
            ...
            <div  id="tabs-2">
                ...
                <p>
                <?php
                $sql = "SELECT COUNT(*) AS total FROM kmrlform where status=''";
                $result = $conn->query($sql);
                $row = $result->fetch_assoc();
                $total_records = $row['total'];
                $limit = 5;
                ...
                $start_from = ($page - 1) * $limit;
                $total_pages = ceil($total_records / $limit);
            $sql1 = "SELECT name,email,status FROM kmrlform where status='' LIMIT $start_from, $limit";
            $result1 = $conn->query($sql1);
            $html = "<table class='paging_table' border='1'>";
            while($row =$result1->fetch_assoc() ){
                $html .="<tr>";
        ...
        ?>
            ...
            <div  id="tabs-3">
                <h3>Rejected Students List</h3>
                <br>
                <p>
                <?php
                $sql = "SELECT COUNT(*) AS total FROM kmrlform where status='reject'";
                $result = $conn->query($sql);
                $row = $result->fetch_assoc();
                $total_records = $row['total'];
                $limit = 5;
                ...
                $start_from = ($page - 1) * $limit;
                $total_pages = ceil($total_records / $limit);
            $sql1 = "SELECT name,email,status FROM kmrlform where status='reject' LIMIT $start_from, $limit";
            $result1 = $conn->query($sql1);
            $html = "<table class='paging_table' border='1'>";
            while($row =$result1->fetch_assoc() ){
                $html .="<tr>";
  ...
  $( function() {   
    ...
    $("#tabs").click(function(e){
        tabid = ($("#tabs .ui-tabs-panel:visible").attr("id").split('tabs-'))
       //
       url = document.location.href.split('?');
       document.location = url[0]+"?tab="+tabid[1];
    ...
    if(document.location.href.indexOf('?tab') !== -1) {
        tabtoselect = document.location.href.split('?tab=')

    } else {
        document.location = document.location.href+"?tab="+tabid[1];
    }

    //initalize the selected id display    
    $("#tabs").tabs({
        active: tabtoselect[1]-1
    });
  } );
  </script>
</section>
